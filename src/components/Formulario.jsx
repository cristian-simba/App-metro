    ...
    useEffect(() => {
        if(idMetro)
        {
            (async function (idMetro) {
                try {
                    const respuesta = await (await fetch(`https://65bfcc3325a83926ab95a5dd.mockapi.io/metro/${idMetro}`)).json()
...
const handleSubmit = async(e)=>{
    ...
    try { if(form.id){
        const url = `https://65bfcc3325a83926ab95a5dd.mockapi.io/metro/${form.id}`
        await fetch(url,{
            method:'PUT',
        ...
        const url ="https://65bfcc3325a83926ab95a5dd.mockapi.io/metro"
                    form.id = uuidv4()
        await fetch(url,{
            method:'POST',
            body:JSON.stringify(form),
